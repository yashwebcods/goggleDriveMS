...
const googleLogin = async (req, res) => {
    try {
        ...
        try {
            ...
            console.error('verifyIdToken error:', code, details);

            const isAuthError = code.startsWith('auth/');
            const status = isAuthError ? 401 : 500;
            const message = isAuthError
                ? 'Invalid Google token'
                : 'Google login server misconfiguration';
        ...
        if (!user) {
            ...
            const rawBase = (decoded?.name || localPart).toString().trim() || 'user';
            const base = rawBase.replace(/\s+/g, '').replace(/[^a-zA-Z0-9_.-]/g, '').slice(0, 32) || 'user';

            let username = base;
            for (let i = 0; i < 10; i++) {
                const existing = await User.findOne({ username });
                if (!existing) break;
                username = `${base}${Math.floor(100000 + Math.random() * 900000)}`;
            }

            const stillExists = await User.findOne({ username });
            if (stillExists) {
    ...
    try {
        ...
        if (normalizedRole === 'client') {
            if (creatorRole === 'manager') {
                ...
                assignedManagerEmail = (req.user?.email || '').toString();
            } else {
                if (!managerEmail) {
                    return res.status(400).json({
                        success: false,
                ...
                assignedManagerEmail = (managerUser.email || '').toString();
                                    ...
                                    <tr>
                                        <td style="padding:30px; color:#333333;">
                                            ...
                                            <table cellpadding="0" cellspacing="0" style="margin:16px 0; width:100%; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden;">
                                                ...
                                                <tr>
                                                    <td style="padding:10px 12px; background:#f9fafb; font-weight:bold; width:160px;">Manager</td>
                                                    <td style="padding:10px 12px;">${assignedManagerEmail}</td>
                                                </tr>
                                                ` : ''}
                                                <tr>
