...
const request = async (path, options = {}) => {
  ...
  if (!res.ok) {
    ...
    return {
      success: false,
      status: res.status,
      message: (json?.message ? `${json.message}${extraError}` : undefined) || (rawSnippet ? `Request failed (HTTP ${res.status}): ${rawSnippet}` : `Request failed (HTTP ${res.status})`),
      data: json?.data,
      raw,
    };
  }
