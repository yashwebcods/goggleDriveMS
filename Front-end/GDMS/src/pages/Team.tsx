...
const Team = () => {
  ...
  const canUseAdminSummary = roleNormalized === 'admin' || roleNormalized === 'superadmin';
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const result = canUseAdminSummary
        ? await userService.getAdminSummary(token)
        : await userService.getTeamOverview(token);
      if (!result?.success) {
    ...
    load();
  }, [canUseAdminSummary, navigate, token]);
  ...
  const content = useMemo(() => {
    ...
    if (data?.users && data?.counts) {
      const counts = data?.counts || {};
      const admins: any[] = data?.users?.admins || [];
      const managers: any[] = data?.users?.managers || [];
      const clients: any[] = data?.users?.clients || [];
      const superadmins: any[] = data?.users?.superadmins || [];

      const renderSummaryTable = (title: string, rows: any[], showCreatedBy: boolean) => (
      ...
      return (
        <div className="space-y-4">
          ...
          {renderSummaryTable('Admins', admins, false)}
          {renderSummaryTable('Managers', managers, false)}
          {renderSummaryTable('Clients', clients, true)}
          {renderSummaryTable('Super Admins', superadmins, false)}
        </div>
      );
    }
