...
const TopHeader = ({ user, onLogout }: Props) => {
  ...
  const canSeeAddMember = role === 'admin' || role === 'superadmin' || role === 'manager';
  const canSeeTeam =
    role === 'admin' || role === 'superadmin' || role === 'manager' || role === 'client';
  const canSeeAccessControl = role === 'admin' || role === 'superadmin' || role === 'manager';
          ...
          <div className="absolute left-0 top-0 h-full w-64 max-w-[80%] bg-white shadow-2xl border-r border-gray-200 p-4 flex flex-col">
            ...
            <div className="mt-auto pt-4">
              <button
                ...
                className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 text-left"
              >
                Logout
              </button>
            </div>
